...
class UnetConv2D(torch.nn.Module):
    def __init__(self, in_channels, out_channels, is_batchnorm, is_leaky=False):
        ...
        self.bn1 = nn.BatchNorm2d(out_channels) if is_batchnorm else nn.Sequential()
        self.act1 = nn.PReLU(out_channels) if is_leaky else nn.ReLU()
        self.conv2 = nn.Conv2d(out_channels, out_channels, 3, padding=1)
        self.bn2 = nn.BatchNorm2d(out_channels) if is_batchnorm else nn.Sequential()
...
class UnetGatingSignal(torch.nn.Module):
    def __init__(self, in_channels, is_batchnorm, is_leaky=False):
        ...
        self.bn = nn.BatchNorm2d(in_channels) if is_batchnorm else nn.Sequential()
...
class DeepSupAttentionUnet(torch.nn.Module):
    ...
    '''
    def __init__(self, in_channels, out_channels, is_batchnorm=True, is_leaky=False, finalact="sigmoid"):
        super(DeepSupAttentionUnet, self).__init__()
        self.conv1 = UnetConv2D(in_channels, 32, is_batchnorm=is_batchnorm, is_leaky=is_leaky)
        self.conv_scale2 = nn.Conv2d(in_channels, 64, 3, padding=1)
        self.conv2 = UnetConv2D(64+32, 64, is_batchnorm=is_batchnorm, is_leaky=is_leaky)
        self.conv_scale3 = nn.Conv2d(in_channels, 128, 3, padding=1)
        self.conv3 = UnetConv2D(128+64, 128, is_batchnorm=is_batchnorm, is_leaky=is_leaky)
        self.conv_scale4 = nn.Conv2d(in_channels, 256, 3, padding=1)
        self.conv4 = UnetConv2D(256+128, 256, is_batchnorm=is_batchnorm, is_leaky=is_leaky)
        self.center = UnetConv2D(256, 512, is_batchnorm=is_batchnorm, is_leaky=is_leaky)
        ...
        self.up1 = nn.ConvTranspose2d(512, 32, 2, 2)
        ...
        self.up2 = nn.ConvTranspose2d(256+32, 64, 2, 2)
        ...
        self.up3 = nn.ConvTranspose2d(128+64, 32, 2, 2)
        self.up4 = nn.ConvTranspose2d(64+32, 32, 2, 2)
        self.conv6 = UnetConv2D(288, 256, is_batchnorm=is_batchnorm, is_leaky=is_leaky)
        self.conv7 = UnetConv2D(192, 128, is_batchnorm=is_batchnorm, is_leaky=is_leaky)
        self.conv8 = UnetConv2D(96, 64, is_batchnorm=is_batchnorm, is_leaky=is_leaky)
        self.conv9 = UnetConv2D(64, 32, is_batchnorm=is_batchnorm, is_leaky=is_leaky)
        self.pred1 = nn.Conv2d(256, out_channels, 1)
        self.pred2 = nn.Conv2d(128, out_channels, 1)
        self.pred3 = nn.Conv2d(64, out_channels, 1)
    ...
    def forward(self, x):
        scale_img_2 = F.avg_pool2d(x, 2)
        scale_img_3 = F.avg_pool2d(scale_img_2, 2)
        scale_img_4 = F.avg_pool2d(scale_img_3, 2)
...
if __name__=='__main__':
    ...
    print(aunet(torch.zeros(1, 1, 512, 512, device='cuda'))[-1].shape)